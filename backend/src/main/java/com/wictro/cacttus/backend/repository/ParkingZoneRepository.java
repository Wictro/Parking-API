...
public interface ParkingZoneRepository extends CrudRepository<ParkingZone, Long> {
    ...
    @Query("SELECT new com.wictro.cacttus.backend.dto.parkingZone.ParkingZoneDto(p.id, p.name,  p.city.id, p.locationLatitude, p.locationLongitude, p.isOperating) from ParkingZone as p WHERE p.city.id = ?1")
    List<ParkingZoneDto> getAllParkingZonesAsDtoByCity(Long cityId);
